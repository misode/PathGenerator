...
execute as @a[gamemode=creative,scores={path_click=1..},nbt={SelectedItem:{tag:{pathStart:1b}}}] if score markers math matches 2.. at @s run function path:init
...
execute if entity @a[gamemode=creative,nbt={SelectedItem:{tag:{pathPlace:1b}}}] if score markers math matches 2.. if entity @a[tag=visualize] unless entity @e[type=llama,tag=path] run function path:show_path
